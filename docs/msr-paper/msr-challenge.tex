...
\begin{questions}
    \item How many bug-inducing commits does CI fail on? \\ Our findings show that 10\% of the builds fail after the bug-inducing commit is introduced to the code base. For the build immediately before the fix is introduced, 5.26\% of the builds fail. This suggests that the CI pipeline failed to detect the bugs when the bug-inducing commit was first pushed into the code base or before it was fixed. 
\end{questions}
\begin{questions}[resume]
    \item Why aren't one-line bugs caught by the CI? \\ We find that 66.7\% of the failed builds preceding the fix commit introduction have no tests and that this proportion is reduced to 45.5\% after the fix commit introduction. This suggests that in some cases, bug-inducing commits had no test coverage making it impossible for the CI pipeline to detect. 
\end{questions}
